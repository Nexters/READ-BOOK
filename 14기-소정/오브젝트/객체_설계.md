...
맨 처음 코드에서 모든 처리는 Theater.enter()가 담당하고 그 안에 Audience, TicketSeller, Bag, TicketOffice 등의 데이터를 절차적으로 사용했다. 절차 지향에서 Theater.enter()는 프로세스, 나머지는 데이터다. 모든 처리가 하나의 클래스에 위치하고 나머지는 단지 데이터의 역할만 수행한다. 절차적 프로그래밍은 객체가 수동적이며 데이터 변경에 영향을 많이 받는다.

객체지향은 데이터를 사용하는 프로세스가 데이터를 소유하도록 한다. 데이터와 프로세스가 동일한 모듈 내부에 있는 것이다. 의존성이 통제되고 변경에 의한 여파가 전파되지 않게 한다.